...
**「冷蔵庫にある材料」表示（UI）**:
- 買い物リストと同じ`name`の食材が冷蔵庫にある場合、リスト下部に「🏠 冷蔵庫にある材料」として表示
- 差し引き計算はせず、参考情報として表示するのみ
- ユーザーが自分で数量を判断して調整する
...
**在庫差分**: **しない**（MVPでは買い物リストから差し引かない）
**単位換算**: **しない**（MVPはしない）

**削除履歴**: 30日間保持、その後自動削除

**買い物リストとの連携**:
- 冷蔵庫の食材は買い物リスト画面で「冷蔵庫にある材料」として参考表示
- 自動差し引きはせず、ユーザーが自分で判断して数量を調整
...
| 2026-02-05 | 冷蔵庫・買い物リスト仕様変更: quantity型をstringに統一、在庫差分を廃止、「冷蔵庫にある材料」UI表示を追加 | DB/API/UI | いつき |
