...
| 機能 | 理由 |
|------|------|
| Catalog API（6.1.9） | グロナビが「献立表」「レシピ帳」のみのため |
| Payment API（6.1.13） | Catalogがないため購入機能不要 |
| Archive API（6.1.12） | マイページがないため履歴閲覧画面なし |
...
### MVP UIの補足
- グロナビ: **献立表** / **レシピ帳** の2タブのみ
- マイページなし → ログアウト・通知・ユーザー情報は**ハンバーガーメニュー**に集約
- ヘッダー右上: `🧊 ☰`（冷蔵庫 / ハンバーガーメニュー）
- ハンバーガーメニューはMVP限定（マイページ代わり）

### 将来UI（マイページ復活後）
- ヘッダー右上: `🧊 🔔`（冷蔵庫 / 通知）
- フッターにマイページ追加 → ログアウト・ユーザー情報はマイページへ移動
- ハンバーガーメニューは廃止
...
|------|------|
| 初期データ数 | 12セット / 約80レシピ |
| 識別方法 | `origin: "official"` または「公式」タグ |
| 投入方法 | DB直接投入（Catalog API不使用） |
...
| 編集 | ❌ | 403 FORBIDDEN |
| 削除 | ❌ | 403 FORBIDDEN |
| コピーして自分用に編集 | ✅ | origin: "created" として新規作成 |
| 非表示 | ❌ | カタログ追加後に解禁 |
...
| 2026-02-06 | Out of scope更新（Catalog/Payment/Archive追加）、MVP UIの補足追記、公式スターターセット仕様追加 | - |
