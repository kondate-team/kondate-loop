...
| レシピ帳へ保存 | ○ | ○ | ○ | ○ | × |
| 献立にセット適用 | ○ | ○ | ○ | ○ | × |
| テキスト、URLからレシピ生成 | △（月10） | △（月100） | △（月10） | △（月100） | × |
| 買い物リスト生成/編集 | ○ | ○ | ○ | ○ | × |
| 冷蔵庫の編集 | ○ | ○ | ○ | ○ | × |
| レシピ/セットをカタログへ公開 | × | × | ○ | ○ | × |
| 公開ルール「有料」を選べる | × | × | △ | ○ | × |
...
**例外的に「必須じゃなくできる」ケース（オフライン）**:
- 設計次第では、一部だけオフライン対応は可能
- すでに取得済みのレシピ一覧を端末にキャッシュして、その範囲内だけローカル検索する
- 自分のレシピ帳（RecipeBookEntry）だけをオフライン表示
- 再接続時に同期
---
